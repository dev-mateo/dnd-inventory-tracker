...
class Remove(commands.Cog):
    ...
    async def remove(self, interaction: Interaction, user: Member, category: Choice[str], item: str) -> None:
        
        query = {"user": str(user.id)}
        new_value = {"$pull": { category.value: item }}
        self.col.update_one(query, new_value)

        
        embed = Embed(
            title="Remove",
            description=f"> `{item}` has been removed from {user.mention}"
        )
